...
- Exposed endpoints: `POST /chat`, `POST /upload-document`, `GET /auth/linkedin/login`, `GET /auth/linkedin/callback`, `GET /auth/linkedin/me`, `POST /auth/linkedin/import`, `POST /auth/linkedin/logout`, and `GET /health`. Keep the OpenAPI schema accurate — every request/response model must be a Pydantic model.
