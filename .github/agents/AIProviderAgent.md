...
## Purpose
Abstracts the underlying AI provider (OpenAI, local/mock, etc.) behind a common interface so providers can be swapped without rewriting business logic.
...
- "Using AIProviderAgent, implement <specific task in its domain>."
- "Ask AIProviderAgent to review <artifact> for compliance with its architecture and security rules."
